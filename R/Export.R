...
exportExposures <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "PMSstudy")
  tcosOfInterest <- read.csv(pathToCsv, stringsAsFactors = FALSE)
  pathToCsv <- system.file("settings", "CohortsToCreate.csv", package = "PMSstudy")
  ...
  createExposureRow <- function(exposureId) {
    ...
    cohortFileName <- system.file("cohorts", paste0(name, ".json"), package = "PMSstudy")
...
exportOutcomes <- function(outputFolder, exportFolder) {
  ...
  pathToCsv <- system.file("settings", "NegativeControls.csv", package = "PMSstudy")
...
exportDiagnostics <- function(outputFolder,
  ...
  cluster <- ParallelLogger::makeCluster(min(4, maxCores))
  ParallelLogger::clusterRequire(cluster, "PMSstudy")
